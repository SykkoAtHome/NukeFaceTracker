...
function baseUvPoint(id){
  ...
  const inner=BASE_UV_LAYOUT[left?362:133], outer=BASE_UV_LAYOUT[left?263:33], top=BASE_UV_LAYOUT[left?386:159], bottom=BASE_UV_LAYOUT[left?374:145];
  const center=[(inner[0]+outer[0]+top[0]+bottom[0])*.25,(inner[1]+outer[1]+top[1]+bottom[1])*.25];
  const rx=[(outer[0]-inner[0])*.14,(outer[1]-inner[1])*.14], ry=[(top[0]-bottom[0])*.16,(top[1]-bottom[1])*.16];
...
function bounds(){if(viewMode==='free3d')return FREE3D_BOUNDS; if(viewMode==='unwrap')return UNWRAP_BOUNDS; let minX=Infinity,maxX=-Infinity,minY=Infinity,maxY=-Infinity; for(let i=0;i<MAX_LANDMARKS;i++){const p=project(landmarkPoint(i),i); minX=Math.min(minX,p[0]); maxX=Math.max(maxX,p[0]); minY=Math.min(minY,p[1]); maxY=Math.max(maxY,p[1]);} return {minX,maxX,minY,maxY};}
